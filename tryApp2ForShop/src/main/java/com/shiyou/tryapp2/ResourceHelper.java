				...
				if (item.tag.equals(Define.TAG_RING))
					RequestManager.loadGoodsDetail(mContext, mUserKey, item.id, new MyRequestCallback()
					{
						...
						void onResponseSuccess(BaseResponse response)
						{
							...
							AndroidUtils.MainHandler.post(new DownloadGoodsDetailResourceRunnable(gdResponse.data,
				...
				else if (item.tag.equals(Define.TAG_COUPLE))
				{
					RequestManager.loadCoupleRingDetail(mContext, mUserKey, item.id, new MyRequestCallback()
					{
						...
						void onResponseSuccess(BaseResponse response)
						{
							...
							AndroidUtils.MainHandler.post(new DownloadGoodsDetailResourceRunnable(crdResponse.data,
									new OnResourceDownloadCallback()
									{
										@Override
										public void onDownloadFinished(Object data)
...
//							fileInfo.url = goodsDetail.thumb;
...
//						}
					...
					case 1:// 下载商品相册图
						...
						{
							goodsDetail.thumb_url2=new ImageInfo[goodsDetail.thumb_url.length];
							for (int i=0;i<goodsDetail.thumb_url.length;i++){
								goodsDetail.thumb_url2[i].url=goodsDetail.thumb_url[i];
							}
					...
					case 2:// 下载商品模型资源
						...
						{
							goodsDetail.model_info2=new FileInfo();
							goodsDetail.model_info2.url=goodsDetail.model_info;
							FileInfo fileInfo = goodsDetail.model_info2;
...
//						{
//							FileInfo fileInfo=new FileInfo();
//							fileInfo.url = coupleRingGoodsDetail.thumb;
//							fileInfo.path = getGoodsImageDirectory(mContext).getAbsolutePath() + File.separator
//									+ FileUtils.getFileName(fileInfo.url);
...
//						}
					...
					case 1:// 下载商品相册图
						...
						{
							FileInfo[] fileInfos = coupleRingGoodsDetail.thumb_url2;
							for (FileInfo fileInfo : fileInfos)
							{
								fileInfo.path = getGoodsImageDirectory(mContext).getAbsolutePath() + File.separator
										+ FileUtils.getFileName(fileInfo.url);
					...
					case 2:// 下载男戒模型资源
						downloadStep++;
						if (coupleRingGoodsDetail.model_infos != null && coupleRingGoodsDetail.model_infos2.men != null)
						{
							FileInfo fileInfo = coupleRingGoodsDetail.model_infos2.men;
							fileInfo.path = getTempFileDirectory(mContext).getAbsolutePath() + File.separator
									+ FileUtils.getFileName(fileInfo.url);
							FileDownloadHelper.checkAndDownloadIfNeed(mContext, coupleRingGoodsDetail, fileInfo,
									new OnFileDownloadCallback()
					...
					case 3:// 下载女戒模型资源
						downloadStep++;
						if (coupleRingGoodsDetail.model_infos != null && coupleRingGoodsDetail.model_infos2.wmen != null)
						{
							FileInfo fileInfo = coupleRingGoodsDetail.model_infos2.wmen;
							fileInfo.path = getTempFileDirectory(mContext).getAbsolutePath() + File.separator
									+ FileUtils.getFileName(fileInfo.url);
							FileDownloadHelper.checkAndDownloadIfNeed(mContext, coupleRingGoodsDetail, fileInfo,
									new OnFileDownloadCallback()
